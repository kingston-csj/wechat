...
public class MultiFlowPaneCopyExample extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        primaryStage.setTitle("Multi-FlowPane Copy Example");
        primaryStage.setScene(scene);
        primaryStage.show();

        // 监听场景的键盘事件
        scene.setOnKeyPressed(event -> handleKeyPress(event, container));
    }
     ...
     * 处理键盘事件
     */
    private void handleKeyPress(KeyEvent event, VBox container) {
        // 检查是否按下了Ctrl+C
        if (event.isControlDown() && event.getCode() == KeyCode.C) {
            // 获取当前选中的FlowPane
            FlowPane selectedFlowPane = getSelectedFlowPane(container);

            if (selectedFlowPane != null) {
                // 获取FlowPane的内容
                String content = getFlowPaneContent(selectedFlowPane);

                // 将内容复制到剪贴板
                copyToClipboard(content);
                System.out.println("Copied: " + content); // 打印复制的内容（可选）
            }
        }
    }
     ...
     * 获取当前选中的FlowPane
     */
    private FlowPane getSelectedFlowPane(VBox container) {
        // 遍历容器中的所有子节点
        for (Node node : container.getChildren()) {
            if (node instanceof FlowPane && node.isFocused()) {
                return (FlowPane) node;
            }
        }
        return null; // 如果没有选中的FlowPane，返回null
    }
